...
public class AppTrabalho {
    ...
    public static void cruzadinhakkkXD() {
        ...
        for (int i = 0; i < 1; i++) {
            System.out.println("\nDigite seu nome para aparecer no placar: ");
            nome[i] = prompt.nextLine();
        }

        // Configuração do jogo de adivinhação de palavras
        System.out.println("Acerte a palavra certa");
        String[] palavrasecreta = { "antedeguemon", "betoneira", "hylux", "airfryer" };
        ...
        while (!palavravalida) {
            ...
            if (letra.length() > 1) {
                System.out.println("Digita só uma letra BURRO");
                tentativas = tentativas + 1;
            }
            if (!Character.isLetter(letrachar)) {
                System.out.print("Tu só pode digitar letra IMBECIL");
            }
